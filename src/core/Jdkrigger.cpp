...
{
public:
    JDKRIG_DISABLE_COPY_MOVE_DEFAULT(JdkriggerPrivate)


    inline explicit JdkriggerPrivate(Controller *controller) : controller(controller) {}


    inline ~JdkriggerPrivate()
    {
        delete timer;
...
#   ifdef JDKRIG_FEATURE_API
    void getJdkrigger(rapidjson::Value &reply, rapidjson::Document &doc, int) const
    {
...
jdkrig::Jdkrigger::Jdkrigger(Controller *controller)
    : d_ptr(new JdkriggerPrivate(controller))
{
...
jdkrig::Jdkrigger::~Jdkrigger()
{
    delete d_ptr;
}


bool jdkrig::Jdkrigger::isEnabled() const
{
    return d_ptr->enabled;
}


bool jdkrig::Jdkrigger::isEnabled(const Algorithm &algorithm) const
{
    return std::find(d_ptr->algorithms.begin(), d_ptr->algorithms.end(), algorithm) != d_ptr->algorithms.end();
}


const jdkrig::Algorithms &jdkrig::Jdkrigger::algorithms() const
{
    return d_ptr->algorithms;
}


const std::vector<jdkrig::IBackend *> &jdkrig::Jdkrigger::backends() const
{
    return d_ptr->backends;
}


jdkrig::Job jdkrig::Jdkrigger::job() const
{
...
void jdkrig::Jdkrigger::execCommand(char command)
{
    switch (command) {
    case 'h':
    case 'H':
...
void jdkrig::Jdkrigger::setEnabled(bool enabled)
{
    ...
    if (d_ptr->controller->config()->isPauseOnBattery() && d_ptr->battery_power && enabled) {
        LOG_INFO("%s " YELLOW_BOLD("can't resume while on battery power"), Tags::jdkrigger());
    ...
    if (enabled) {
        LOG_INFO("%s " GREEN_BOLD("resumed"), Tags::jdkrigger());
    }
    else {
        if (d_ptr->battery_power) {
            LOG_INFO("%s " YELLOW_BOLD("paused"), Tags::jdkrigger());
        }
        else {
            LOG_INFO("%s " YELLOW_BOLD("paused") ", press " MAGENTA_BG_BOLD(" r ") " to resume", Tags::jdkrigger());
        }
    }
...
void jdkrig::Jdkrigger::setJob(const Job &job, bool donate)
{
...
void jdkrig::Jdkrigger::onConfigChanged(Config *config, Config *previousConfig)
{
    d_ptr->rebuild();
...
void jdkrig::Jdkrigger::onTimer(const Timer *)
{
    ...
    auto autoPause = [this](bool &state, bool pause, const char *pauseMessage, const char *activeMessage)
    {
        if ((pause && !state) || (!pause && state)) {
            LOG_INFO("%s %s", Tags::jdkrigger(), pause ? pauseMessage : activeMessage);
...
#ifdef JDKRIG_FEATURE_API
void jdkrig::Jdkrigger::onRequest(IApiRequest &request)
{
    if (request.method() == IApiRequest::METHOD_GET) {
        if (request.type() == IApiRequest::REQ_SUMMARY) {
            request.accept();

            d_ptr->getJdkrigger(request.reply(), request.doc(), request.version());
...
#ifdef JDKRIG_ALGO_RANDOMX
void jdkrig::Jdkrigger::onDatasetReady()
{
    if (!Rx::isReady(job())) {
        return;
    }
