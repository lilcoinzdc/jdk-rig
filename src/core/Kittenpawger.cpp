...
{
public:
    KITTENPAW_DISABLE_COPY_MOVE_DEFAULT(KittenpawgerPrivate)


    inline explicit KittenpawgerPrivate(Controller *controller) : controller(controller) {}


    inline ~KittenpawgerPrivate()
    {
        delete timer;
...
#   ifdef KITTENPAW_FEATURE_API
    void getKittenpawger(rapidjson::Value &reply, rapidjson::Document &doc, int) const
    {
...
kittenpaw::Kittenpawger::Kittenpawger(Controller *controller)
    : d_ptr(new KittenpawgerPrivate(controller))
{
...
kittenpaw::Kittenpawger::~Kittenpawger()
{
    delete d_ptr;
}


bool kittenpaw::Kittenpawger::isEnabled() const
{
    return d_ptr->enabled;
}


bool kittenpaw::Kittenpawger::isEnabled(const Algorithm &algorithm) const
{
    return std::find(d_ptr->algorithms.begin(), d_ptr->algorithms.end(), algorithm) != d_ptr->algorithms.end();
}


const kittenpaw::Algorithms &kittenpaw::Kittenpawger::algorithms() const
{
    return d_ptr->algorithms;
}


const std::vector<kittenpaw::IBackend *> &kittenpaw::Kittenpawger::backends() const
{
    return d_ptr->backends;
}


kittenpaw::Job kittenpaw::Kittenpawger::job() const
{
...
void kittenpaw::Kittenpawger::execCommand(char command)
{
    switch (command) {
    case 'h':
    case 'H':
...
void kittenpaw::Kittenpawger::pause()
{
    d_ptr->active = false;
...
void kittenpaw::Kittenpawger::setEnabled(bool enabled)
{
    ...
    if (d_ptr->controller->config()->isPauseOnBattery() && d_ptr->battery_power && enabled) {
        LOG_INFO("%s " YELLOW_BOLD("can't resume while on battery power"), Tags::kittenpawger());
    ...
    if (enabled) {
        LOG_INFO("%s " GREEN_BOLD("resumed"), Tags::kittenpawger());
    }
    else {
        if (d_ptr->battery_power) {
            LOG_INFO("%s " YELLOW_BOLD("paused"), Tags::kittenpawger());
        }
        else {
            LOG_INFO("%s " YELLOW_BOLD("paused") ", press " MAGENTA_BG_BOLD(" r ") " to resume", Tags::kittenpawger());
        }
    }
...
void kittenpaw::Kittenpawger::setJob(const Job &job, bool donate)
{
...
void kittenpaw::Kittenpawger::stop()
{
    Nonce::stop();
...
void kittenpaw::Kittenpawger::onConfigChanged(Config *config, Config *previousConfig)
{
    d_ptr->rebuild();
...
void kittenpaw::Kittenpawger::onTimer(const Timer *)
{
    ...
    auto autoPause = [this](bool &state, bool pause, const char *pauseMessage, const char *activeMessage)
    {
        if ((pause && !state) || (!pause && state)) {
            LOG_INFO("%s %s", Tags::kittenpawger(), pause ? pauseMessage : activeMessage);
...
#ifdef KITTENPAW_FEATURE_API
void kittenpaw::Kittenpawger::onRequest(IApiRequest &request)
{
    if (request.method() == IApiRequest::METHOD_GET) {
        if (request.type() == IApiRequest::REQ_SUMMARY) {
            request.accept();

            d_ptr->getKittenpawger(request.reply(), request.doc(), request.version());
...
#ifdef KITTENPAW_ALGO_RANDOMX
void kittenpaw::Kittenpawger::onDatasetReady()
{
    if (!Rx::isReady(job())) {
        return;
    }
