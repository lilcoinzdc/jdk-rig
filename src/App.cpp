...
kittenpaw::App::App(Process *process)
{
...
int kittenpaw::App::exec()
{
    if (!m_controller->isReady()) {
        LOG_EMERG("no valid configuration found, try https://kittenpaw.com/wizard");
...
void kittenpaw::App::onConsoleCommand(char command)
{
    if (command == 3) {
...
void kittenpaw::App::kuka(int signum)
{
...
void kittenpaw::App::onSignal(int signum)
{
    switch (signum)
    {
    case SIGHUP:
