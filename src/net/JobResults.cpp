...
{
public:
    JDKRIG_DISABLE_COPY_MOVE_DEFAULT(JobResultsPrivate)
...
void jdkrig::JobResults::done(const Job &job)
{
    submit(JobResult(job));
}


void jdkrig::JobResults::setListener(IJobResultListener *listener, bool hwAES)
{
...
void jdkrig::JobResults::submit(const Job &job, uint32_t nonce, const uint8_t *result)
{
    submit(JobResult(job, nonce, result));
}


void jdkrig::JobResults::submit(const Job& job, uint32_t nonce, const uint8_t* result, const uint8_t* jdkrigger_signature)
{
    submit(JobResult(job, nonce, result, nullptr, nullptr, jdkrigger_signature));
}


void jdkrig::JobResults::submit(const JobResult &result)
{
    assert(handler != nullptr);
...
#if defined(JDKRIG_FEATURE_OPENCL) || defined(JDKRIG_FEATURE_CUDA)
void jdkrig::JobResults::submit(const Job &job, uint32_t *results, size_t count, uint32_t device_index)
{
    if (handler) {
