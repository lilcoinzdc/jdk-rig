...
{
public:
    KITTENPAW_DISABLE_COPY_MOVE_DEFAULT(JobResultsPrivate)
...
void kittenpaw::JobResults::done(const Job &job)
{
    submit(JobResult(job));
}


void kittenpaw::JobResults::setListener(IJobResultListener *listener, bool hwAES)
{
...
void kittenpaw::JobResults::stop()
{
    assert(handler != nullptr);
...
void kittenpaw::JobResults::submit(const Job &job, uint32_t nonce, const uint8_t *result)
{
    submit(JobResult(job, nonce, result));
}


void kittenpaw::JobResults::submit(const Job& job, uint32_t nonce, const uint8_t* result, const uint8_t* kittenpawger_signature)
{
    submit(JobResult(job, nonce, result, nullptr, nullptr, kittenpawger_signature));
}


void kittenpaw::JobResults::submit(const JobResult &result)
{
    assert(handler != nullptr);
...
#if defined(KITTENPAW_FEATURE_OPENCL) || defined(KITTENPAW_FEATURE_CUDA)
void kittenpaw::JobResults::submit(const Job &job, uint32_t *results, size_t count, uint32_t device_index)
{
    if (handler) {
