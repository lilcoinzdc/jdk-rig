...
kittenpaw::CudaThread::CudaThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
kittenpaw::CudaThread::CudaThread(uint32_t index, nvid_ctx *ctx) :
...
bool kittenpaw::CudaThread::isEqual(const CudaThread &other) const
{
...
rapidjson::Value kittenpaw::CudaThread::toJSON(rapidjson::Document &doc) const
{
