...
kittenpaw::CudaBaseRunner::CudaBaseRunner(size_t id, const CudaLaunchData &data) :
    m_data(data),
    m_threadId(id)
{
}
...
kittenpaw::CudaBaseRunner::~CudaBaseRunner()
{
    CudaLib::release(m_ctx);
}


bool kittenpaw::CudaBaseRunner::init()
{
...
bool kittenpaw::CudaBaseRunner::set(const Job &job, uint8_t *blob)
{
    m_height = job.height();
    m_target = job.target();
...
size_t kittenpaw::CudaBaseRunner::intensity() const
{
    return m_data.thread.threads() * m_data.thread.blocks();
}


bool kittenpaw::CudaBaseRunner::callWrapper(bool result) const
{
    if (!result) {
