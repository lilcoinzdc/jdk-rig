...
void kittenpaw::FindSharesKernel::enqueue(cl_command_queue queue, size_t threads)
{
...
void kittenpaw::FindSharesKernel::setArgs(cl_mem hashes, cl_mem shares)
{
...
void kittenpaw::FindSharesKernel::setTarget(uint64_t target)
{
    setArg(1, sizeof(uint64_t), &target);
}


void kittenpaw::FindSharesKernel::setNonce(uint32_t nonce)
{
