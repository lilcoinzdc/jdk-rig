...
{
public:
    KITTENPAW_DISABLE_COPY_MOVE(LogPrivate)
...
void kittenpaw::Log::add(ILogBackend *backend)
{
    assert(d != nullptr);
...
void kittenpaw::Log::init()
{
    d = new LogPrivate();
}


void kittenpaw::Log::print(const char *fmt, ...)
{
    if (!d) {
        return;
    }
...
void kittenpaw::Log::print(Level level, const char *fmt, ...)
{
    if (!d) {
        return;
    }
