...
kittenpaw::Algorithm::Algorithm(const rapidjson::Value &value) :
    m_id(parse(value.GetString()))
{
}


kittenpaw::Algorithm::Algorithm(uint32_t id) :
    m_id(kAlgorithmNames.count(id) ? static_cast<Id>(id) : INVALID)
{
}


const char *kittenpaw::Algorithm::name() const
{
    if (!isValid()) {
        return kINVALID;
    }
...
rapidjson::Value kittenpaw::Algorithm::toJSON() const
{
...
rapidjson::Value kittenpaw::Algorithm::toJSON(rapidjson::Document &) const
{
    return toJSON();
}


kittenpaw::Algorithm::Id kittenpaw::Algorithm::parse(const char *name)
{
...
size_t kittenpaw::Algorithm::count()
{
    return kAlgorithmNames.size();
}


std::vector<kittenpaw::Algorithm> kittenpaw::Algorithm::all(const std::function<bool(const Algorithm &algo)> &filter)
{
