...
kittenpaw::DaemonClient::DaemonClient(int id, IClientListener *listener) :
...
kittenpaw::DaemonClient::~DaemonClient()
{
    delete m_timer;
    delete m_ZMQSocket;
}
...
void kittenpaw::DaemonClient::deleteLater()
{
...
bool kittenpaw::DaemonClient::disconnect()
{
...
bool kittenpaw::DaemonClient::isTLS() const
{
#   ifdef KITTENPAW_FEATURE_TLS
    return m_pool.isTLS();
#   else
...
int64_t kittenpaw::DaemonClient::submit(const JobResult &result)
{
...
#   ifdef KITTENPAW_PROXY_PROJECT
    ...
    if (m_blocktemplate.hasKittenpawgerSignature() && result.sig) {
...
#   else
    ...
    if (m_blocktemplate.hasKittenpawgerSignature()) {
        Cvt::toHex(data + sig_offset * 2, 128, result.kittenpawgerSignature(), 64);
    }
...
void kittenpaw::DaemonClient::connect()
{
...
void kittenpaw::DaemonClient::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void kittenpaw::DaemonClient::setPool(const Pool &pool)
{
...
void kittenpaw::DaemonClient::onHttpData(const HttpData &data)
{
    if (data.status != 200) {
        return retry();
    }
...
void kittenpaw::DaemonClient::onTimer(const Timer *)
{
    if (m_pool.zmq_port() >= 0) {
        m_prevHash = nullptr;
...
void kittenpaw::DaemonClient::onResolved(const DnsRecords &records, int status, const char* error)
{
    m_dns.reset();
...
bool kittenpaw::DaemonClient::isOutdated(uint64_t height, const char *hash) const
{
    return m_job.height() != height || m_prevHash != hash || Chrono::steadyMSecs() >= m_jobSteadyMs + m_pool.jobTimeout();
}


bool kittenpaw::DaemonClient::parseJob(const rapidjson::Value &params, int *code)
{
...
#   ifdef KITTENPAW_PROXY_PROJECT
    const size_t k = m_blocktemplate.offset(BlockTemplate::KITTENPAWGER_TX_PREFIX_OFFSET);
    job.setKittenpawgerTx(
        m_blocktemplate.blob() + k,
        m_blocktemplate.blob() + m_blocktemplate.offset(BlockTemplate::KITTENPAWGER_TX_PREFIX_END_OFFSET),
        ...
        m_blocktemplate.kittenpawgerTxMerkleTreeBranch(),
...
#       else
        ...
        if (!generate_key_derivation(m_blocktemplate.blob(BlockTemplate::TX_PUBKEY_OFFSET), secret_viewkey, derivation, nullptr)) {
            return jobError("Failed to generate key derivation for kittenpawger signature.");
        }
...
bool kittenpaw::DaemonClient::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
int64_t kittenpaw::DaemonClient::getBlockTemplate()
{
    using namespace rapidjson;
    Document doc(kObjectType);
...
int64_t kittenpaw::DaemonClient::rpcSend(const rapidjson::Document &doc, const std::map<std::string, std::string> &headers)
{
...
void kittenpaw::DaemonClient::retry()
{
    m_failures++;
...
void kittenpaw::DaemonClient::send(const char *path)
{
...
void kittenpaw::DaemonClient::setState(SocketState state)
{
    if (m_state == state) {
        return;
    }
...
void kittenpaw::DaemonClient::onZMQConnect(uv_connect_t* req, int status)
{
...
void kittenpaw::DaemonClient::onZMQRead(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf)
{
...
void kittenpaw::DaemonClient::onZMQClose(uv_handle_t* handle)
{
...
void kittenpaw::DaemonClient::onZMQShutdown(uv_handle_t* handle)
{
    DaemonClient* client = getClient(handle->data);
    if (client) {
#       ifdef APP_DEBUG
...
void kittenpaw::DaemonClient::ZMQConnected()
{
#   ifdef APP_DEBUG
...
bool kittenpaw::DaemonClient::ZMQWrite(const char* data, size_t size)
{
...
void kittenpaw::DaemonClient::ZMQRead(ssize_t nread, const uv_buf_t* buf)
{
    if (nread <= 0) {
...
void kittenpaw::DaemonClient::ZMQParse()
{
#   ifdef APP_DEBUG
    std::vector<char> msg;
#   endif
...
#   endif
    ...
    // Clear previous hash and check daemon height to guarantee that kittenpaw will call get_block_template RPC later
...
bool kittenpaw::DaemonClient::ZMQClose(bool shutdown)
{
    if ((m_ZMQConnectionState == ZMQ_NOT_CONNECTED) || (m_ZMQConnectionState == ZMQ_DISCONNECTING)) {
        if (shutdown) {
