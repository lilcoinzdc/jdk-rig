...
kittenpaw::Pool::Pool(const char *url) :
...
kittenpaw::Pool::Pool(const char *host, uint16_t port, const char *user, const char *password, const char* spendSecretKey, int keepAlive, bool nicehash, bool tls, Mode mode) :
...
kittenpaw::Pool::Pool(const rapidjson::Value &object) :
    m_flags(1 << FLAG_ENABLED),
    m_pollInterval(kDefaultPollInterval),
    m_jobTimeout(kDefaultJobTimeout),
...
#ifdef KITTENPAW_FEATURE_BENCHMARK
kittenpaw::Pool::Pool(const std::shared_ptr<BenchConfig> &benchmark) :
...
kittenpaw::BenchConfig *kittenpaw::Pool::benchmark() const
{
...
uint32_t kittenpaw::Pool::benchSize() const
{
...
bool kittenpaw::Pool::isEnabled() const
{
#   ifndef KITTENPAW_FEATURE_TLS
    if (isTLS()) {
        return false;
    }
#   endif

#   ifndef KITTENPAW_FEATURE_HTTP
...
bool kittenpaw::Pool::isEqual(const Pool &other) const
{
...
kittenpaw::IClient *kittenpaw::Pool::createClient(int id, IClientListener *listener) const
{
...
rapidjson::Value kittenpaw::Pool::toJSON(rapidjson::Document &doc) const
{
...
std::string kittenpaw::Pool::printableName() const
{
...
#ifdef APP_DEBUG
void kittenpaw::Pool::print() const
{
...
void kittenpaw::Pool::setKeepAlive(const rapidjson::Value &value)
{
    if (value.IsInt()) {
