...
jdkrig::HttpServer::HttpServer(const std::shared_ptr<IHttpListener> &listener) :
    m_listener(listener)
{
}


jdkrig::HttpServer::~HttpServer()
{
    HttpContext::closeAll();
}


void jdkrig::HttpServer::onConnection(uv_stream_t *stream, uint16_t)
{
