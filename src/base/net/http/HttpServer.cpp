...
kittenpaw::HttpServer::HttpServer(const std::shared_ptr<IHttpListener> &listener) :
    m_listener(listener)
{
}


kittenpaw::HttpServer::~HttpServer()
{
    HttpContext::closeAll();
}


void kittenpaw::HttpServer::onConnection(uv_stream_t *stream, uint16_t)
{
