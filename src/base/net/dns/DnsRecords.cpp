...
const kittenpaw::DnsRecord &kittenpaw::DnsRecords::get(DnsRecord::Type prefered) const
{
...
size_t kittenpaw::DnsRecords::count(DnsRecord::Type type) const
{
    if (type == DnsRecord::A) {
        return m_ipv4.size();
    }
...
void kittenpaw::DnsRecords::clear()
{
    m_ipv4.clear();
    m_ipv6.clear();
}
...
void kittenpaw::DnsRecords::parse(addrinfo *res)
{
    clear();
